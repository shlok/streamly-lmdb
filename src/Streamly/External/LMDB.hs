...
data WriteOptions = WriteOptions
  { -- | The number of key-value pairs per write transaction.
    ...
    writeFailureFold :: !(Fold IO (LMDB_Error, ByteString, ByteString) ())
  }

-- | By default, we use a write transaction size of 1 (one write transaction for each key-value
-- pair), allow overwriting, don’t assume that the input data is already ordered, don’t use
-- unsafe FFI calls, and use 'throwOnWriteFailure' as the failure fold.
...
defaultWriteOptions =
  WriteOptions
    { writeTransactionSize = 1,
      ...
      writeFailureFold = throwOnWriteFailure
    }

-- | Throw an exception upon write failure.
throwOnWriteFailure :: Fold IO (LMDB_Error, ByteString, ByteString) ()
throwOnWriteFailure =
  Fold (\() (e, _, _) -> throw e) (return $ Partial ()) (const $ return ())

-- | Gracefully stop upon write failure.
stopOnWriteFailure :: Fold IO (LMDB_Error, ByteString, ByteString) ()
stopOnWriteFailure =
  Fold (\() (_, _, _) -> return $ Done ()) (return $ Partial ()) (const $ return ())

-- | Ignore write failures.
ignoreWriteFailures :: Fold IO (LMDB_Error, ByteString, ByteString) ()
ignoreWriteFailures =
  Fold (\() (_, _, _) -> return $ Partial ()) (return $ Partial ()) (const $ return ())
            ...
            ( \(threadId, iter, mtxn, fstep) (k, v) -> do
                ...
                eChunkSz <- liftIO $
                  unsafeUseAsCStringLen k $ \(kp, kl) -> unsafeUseAsCStringLen v $ \(vp, vl) ->
                      ...
                      ( \(e :: LMDB_Error) ->
                          -- (*) Discard LMDB error if OverwriteAllowSame was specified and the
                  ...
                  Left e -> do
                    case fstep of
                      ...
                      Partial s -> do
                        ...
                        case fstep' of
                          Done () -> do
                            -- The failure fold completed with Done; complete the outer writeLMDB
                            -- fold with Done as well.
                            runIOFinalizer ref
                            return $ Done ()
                          Partial _ ->
