...
import Streamly.External.LMDB (Mode, ReadWrite, WriteOptions(..),
...
tests db = [ testReadLMDB db, testUnsafeReadLMDB db, testWriteLMDB db, testWriteLMDB_2 db ]
...
testWriteLMDB res = testProperty "writeLMDB" . monadicIO $ do
    ...
    let fol' = writeLMDB db $ defaultWriteOptions { writeTransactionSize = chunkSz, noOverwrite = False }
...
testWriteLMDB_2 res = testProperty "writeLMDB_2" . monadicIO $ do
    ...
    -- TODO: Run with new "bound" functionality in streamly.
    let fol' = writeLMDB db $ defaultWriteOptions { writeTransactionSize = chunkSz, noOverwrite = True }
